...
<main class="mt-5 w-96 h-full">
	<div
		class="inline-block align-middle w-full max-w-md bg-white border border-gray-200 rounded-lg shadow p-8 dark:bg-gray-800 dark:border-gray-700"
	>
		<form class="space-y-6" action="?/signin" method="post">
			...
			<div>
				{#if form?.status == 400}
					<div class="text-red-500 text-sm font-medium">{form?.message}</div>
				{/if}
			</div>
